...
public partial class MainWindow : Window {
   ...
   internal Viewport Viewport => mViewport!;
   #endregion

   #region Implementation -------------------------------------------
   void OnButtonClicked (object sender, EventArgs e) {
      if (sender is not ToggleButton btn || mViewport is null) return;
      if (!Enum.TryParse ($"{btn.ToolTip}", out ECadOption option)) return;
      Widget? widget = null;
      switch (option) {
         ...
         //   break;
         case ECadOption.Plane: widget = new PlaneWidget (); break;
         default: break;
      }
   ...
   void OnLoaded (object sender, RoutedEventArgs e) {
      ...
      var optionPanel = new StackPanel () { Name = "CadOptions", HorizontalAlignment = HA.Left, Margin = new Thickness (0, 10, 0, 0) };
      ...
      borderStyle.Setters.Add (new Setter (Border.BorderThicknessProperty, new Thickness (0.5)));
